"""Vistas de la API REST y de las plantillas HTML de préstamos."""
...
class PrestamoViewSet(viewsets.ModelViewSet):
    """Expone operaciones CRUD sobre préstamos."""
...
def prestamo_list(request):
    """Muestra el listado de préstamos registrados."""

    prestamos = Prestamo.objects.all()
    return render(request, 'prestamos/list.html', {'prestamos': prestamos})

def prestamo_create(request):
    """Permite registrar un nuevo préstamo desde un formulario HTML."""
...
def prestamo_delete(request, pk):
    """Confirma y elimina un préstamo existente."""
